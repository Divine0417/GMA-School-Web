...
| `TERMII_API_KEY` + `TERMII_SENDER_ID` | termii.com — sender ID must be approved in the Termii dashboard before SMS actually delivers |
...
- Get a real `TERMII_API_KEY` + approved sender ID so SMS credentials actually send.
